 ...
 *   3. Auto-advance status (sent → replied)
 *   4. Fire AI classify + suggest
 *
 * Instantly webhook payload (varies by plan/version):
 *   { event_type, email, reply_text, subject, timestamp, campaign_id, ... }
 *
 * We also accept the v2 format:
 *   { event, data: { from_email, body, subject, timestamp } }
 */
export async function POST(request: Request) {
  try {
    ...
    console.log('[instantly-webhook] received:', JSON.stringify(body).slice(0, 500))

    // Normalize payload — handle both v1 and v2 formats
    const email: string =
      body.email ?? body.from_email ?? body.data?.from_email ?? body.data?.email ?? ''
    const replyText: string =
      body.reply_text ?? body.text ?? body.data?.body ?? body.data?.text ?? ''
    const subject: string =
      body.subject ?? body.data?.subject ?? ''
    const eventType: string =
      body.event_type ?? body.event ?? ''

    // Only process reply events
    if (eventType && !eventType.toLowerCase().includes('reply')) {
      console.log('[instantly-webhook] ignoring non-reply event:', eventType)
      return Response.json({ ok: true })
    }

    if (!email || !replyText) {
      console.log('[instantly-webhook] missing email or reply text, skipping')
      return Response.json({ ok: true })
    }

    const preview = replyText.length > 80 ? replyText.slice(0, 80) + '…' : replyText
    ...
    if (leads && leads.length > 0) {
      ...
      console.log('[instantly-webhook] creating inbound lead for', placeId)
      ...
      if (leadError) {
        console.error('[instantly-webhook] failed to upsert inbound lead:', leadError.message)
      }
    }
    ...
    const { data: dup } = await supabase
      ...
      .gte('sent_at', new Date(new Date(sentAt).getTime() - 30_000).toISOString())
      .limit(1)

    if (dup && dup.length > 0) {
      console.log('[instantly-webhook] duplicate message detected, skipping')
      return Response.json({ ok: true })
    }

    // Detect auto-reply
    if (isAutoReply(replyText)) {
      console.log('[instantly-webhook] auto-reply detected for', placeId)
    ...
    if (convError) {
      console.error('[instantly-webhook] failed to save conversation:', convError.message)
    }
    ...
    if (fullLead.data) {
      ...
      console.log('[instantly-webhook] firing classifyAndSuggest for', placeId)
      classifyAndSuggest(
        ...
        console.error('[instantly-webhook] classify failed:', err.message)
      })
    }

    console.log('[instantly-webhook] saved inbound email for lead', placeId)
    return Response.json({ ok: true })
  } catch (err) {
